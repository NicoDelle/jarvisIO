...
void createMelFilterbank(float filterbank[N_MELS][N_FFT / 2 + 1])
{
    ...
    { return 700.0f * (expf(m / 1127.0f) - 1.0f); };
...
void dct(float *input, float *output, int length)
{
    for (int k = 0; k < N_MFCC; k++)
    {
        ...
        {
            sum += input[n] * cosf((M_PI / length) * (n + 0.5f) * k);
        }
        output[k] = sum * sqrtf(2.0f / length);
    }
}

void extractMFCC(float melspectrogram[N_MELS][NUM_FRAMES], float mfcc[N_MFCC][NUM_FRAMES])
{
    // Temporary buffer for DCT
    float melFrame[N_MELS];
    float dctOutput[N_MFCC];

    // Process each frame
    for (int frame = 0; frame < NUM_FRAMES; frame++)
    {
        // Copy mel spectrogram frame to temporary buffer
        for (int mel = 0; mel < N_MELS; mel++)
        {
            melFrame[mel] = melspectrogram[mel][frame];
        }

        // Apply DCT
        dct(melFrame, dctOutput, N_MELS);

        // Store MFCC coefficients
        for (int coeff = 0; coeff < N_MFCC; coeff++)
        {
            mfcc[coeff][frame] = dctOutput[coeff];
        }
    }
}